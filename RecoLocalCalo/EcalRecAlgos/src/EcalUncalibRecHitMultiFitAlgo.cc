...
EcalUncalibratedRecHit EcalUncalibRecHitMultiFitAlgo::makeRecHit(const EcalDataFrame &dataFrame,
  ...
  for (unsigned int iSample = 0; iSample < nsample; iSample++) {
    ...
    if (gainId == 0 || gainId == 3) {
      pedestal = aped.mean(ecalPh2::gainId1);;
...
/// compute rechits for Phaase2
EcalUncalibratedRecHit EcalUncalibRecHitMultiFitAlgo::makePhase2RecHit(const EcalDataFrame_Ph2 &dataFrame,
  ...
  ecalph2::SampleVector amplitudes;
  ecalph2::SampleGainVector gainsNoise;
  ecalph2::SampleGainVector gainsPedestal;
  ecalph2::SampleGainVector badSamples = ecalph2::SampleGainVector::Zero();
  bool hasSaturation = dataFrame.isSaturated();
  //Dario's question:
  //In DataFormats/EcalDigi/interface/EcalDataFrame_Ph2.h there are no switch infos: to be added?
  bool hasGainSwitch = hasSaturation || dataFrame.hasSwitchToGain6() || dataFrame.hasSwitchToGain1();
